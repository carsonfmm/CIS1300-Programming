 ...
 *   Function: to print out number of days between two given dates typed with dashes, including the starting date and giving the user an option to include the final date or exclude the final date. The user also has the option to include the current date by typing in 'today' as one of the arguments.
 *   Compilation: gcc -ansi -o daysCalculatorE daysCalculatorE.c
 *   Execution: ./daysCalculatorE 14-9-2019/today 9-12-2019/today include(optional)
 */
...
int main ( int argc, char *argv[] ) {
   ...
   int totalYears = yyyy2 - yyyy1;
   int totalDays = (totalYears * 365) - 1;
   for ( j = yyyy1; j <= yyyy2; j++ ) {
      if ( j % 4 == 0 ) {
         if ( j % 400 == 0 || j % 100 != 0 ) {
   ...
   if ( yyyy1 % 4 == 0 || yyyy2 % 4 == 0 ) {
      ...
      int total = (total2 - total1) + totalDays;
